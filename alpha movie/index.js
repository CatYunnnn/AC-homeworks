...
paginator.addEventListener("click", function onPaginatorClicked(event) {
  ...
  if (checkListorCard === 0) {
    renderMovieList(getMoviesByPage(page));
  } else if (checkListorCard === 1) {
    changetoList(getMoviesByPage(page));
  }
});
...
//change to list
function changetoList(index) {
  let content = "";
  index.forEach((item) => {
    // title, image, id
    content += `<div class="row">
    <div class="col-6">${item.title}</div><div class="col-3 ms-auto"><button 
    class="btn btn-primary 
    btn-show-movie" 
    data-bs-toggle="modal" 
    data-bs-target="#movie-modal" 
